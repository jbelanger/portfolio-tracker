// app/components/AddTransactionDialog.tsx
...
import { CsvFileImportType, CryptoCurrencyTransactionDto } from '../types/types';
...
const AddTransactionDialog: React.FC<TransactionDialogProps> = ({ open, onClose, onTransactionAdded, portfolioId, selectedWalletId }) => {
  ...
  const handleImportTransactions = async (): Promise<void> => {
    ...
    const result = await TransactionAPI.uploadCsvTransactions(portfolioId, selectedWalletId, formData, importType);  
    if(result.isOk()) {      
      showSnackbar('Transactions imported successfully.', 'success');
        ...
        console.error('Error importing transactions:', result.error);
