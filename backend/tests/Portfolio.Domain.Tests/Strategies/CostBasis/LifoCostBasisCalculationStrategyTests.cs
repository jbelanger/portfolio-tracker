...
{
    [TestFixture]
    public class LifoCostBasisCalculationStrategyTests
    {
        private LifoCostBasisCalculationStrategy _strategy;

        [SetUp]
        public void SetUp()
        {
            _strategy = new LifoCostBasisCalculationStrategy();
        }
        ...
        public void CalculateCostBasis_ShouldReturnCorrectCostBasis_WhenMultiplePurchaseRecordsExist()
        {
            ...
            // Assert
            result.IsSuccess.Should().BeTrue();
            result.Value.Should().Be(20000); // (1 * 15000) + (0.5 * 10000)
            transaction.ErrorType.Should().Be(ErrorType.None);
            transaction.ErrorMessage.Should().BeEmpty();
        }
